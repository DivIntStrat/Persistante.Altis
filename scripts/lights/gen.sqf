...
for "_i" from 0 to ((count _gen)-1) do
{
    ...
    (_gen select _i) addAction ["Light ON", {[player, true, 1000, 500] call utilities_fnc_switch;},[],1.5,true,true,"player distance2D (_gen select _i) < 10","true",5]; 
    (_gen select _i) addAction ["Light OFF", {[player, false, 1000, 500] call utilities_fnc_switch;},[],1.5,true,true,"player distance2D (_gen select _i) < 10","true",5];
};
